...
import { normalizeIncomingArray } from 'sinuous';
...
export default function each(items, expr) {
  function init(h, parent, afterNode) {
    const { subscribe, root, sample, cleanup, addCleanup } = h;
    ...
    addCleanup(disposer._disposeAll);
  }
...
export function reconcile(
  ...
  fixes: while (loop) {
    ...
    while (a === b) {
      ...
      let mark = _node && _node.nextSibling;
